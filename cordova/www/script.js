 ...
 * Automatically switches between local (development) and remote (production) server URLs.
 */
const LOCAL_SERVER = "http://192.168.1.17:8080/";

/**
 * Helper for building API URLs
 * Usage: api("login.php") → returns correct full URL
 */
function api(endpoint) {
  return LOCAL_SERVER + endpoint;
}
...
function checkout() {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.success) {
        loadCart();
        showMessage(`${response.message} Order ID: ${response.data.orderId}`, "success");
        showSection("account");
      } else {
        showMessage(response.message, "error");
      }
  ...
  products.forEach(function (product) {
    html += `
              <tr>
                  ...
                  <td>
                      <button class="btn btn-secondary" onclick="showEditProductForm(${
                        product.product_id
                      })">Edit</button>
...
function deleteProduct(productId) {
  ...
  $.ajax({
    url: api("adminDeleteProduct.php"),
    type: "POST",
    data: { productId: productId },
    dataType: "json",
    success: function (response) {
